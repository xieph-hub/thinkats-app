...
export async function POST(_req: NextRequest) {
  try {
    // 1) Get current Supabase user from cookies/session
    ...
    // 2) Find or create app-level User row
    ...
    if (!resend) {
      console.warn(
        "[ThinkATS OTP] RESEND_API_KEY missing. OTP email not actually sent.",
      );
    } else {
      try {
        ...
        // We still return ok: true, since OTP exists in DB.
      }
    }
