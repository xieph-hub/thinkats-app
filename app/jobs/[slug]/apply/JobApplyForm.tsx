    ...
    try {
      ...
      if (cvFile && cvFile.size > 0) {
        ...
        if (!uploadRes.ok || uploadData?.error) {
          ...
          // don't hard-fail the application; they might still have a CV link
        } else if (typeof uploadData.url === "string") {
          uploadedCvUrl = uploadData.url;
        }
      }

      // 2) Build JSON payload for /api/jobs/[slug]/apply
      ...
      if (!res.ok || data?.error) {
        setErrorMessage(
          data?.error ||
            `Unexpected error while submitting application (status ${res.status}).`
        );
      } else {
        setSuccessMessage(
