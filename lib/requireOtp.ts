 ...
 * If not, redirect them to /auth/otp.
 */
export async function ensureOtpVerified(returnTo: string = "/ats") {
  ...
  const now = new Date();
  const cutoff = new Date(now.getTime() - 24 * 60 * 60 * 1000); // e.g. OTP valid for 24h after use
  ...
  // If we get here, OTP is considered verified for this session window
}
