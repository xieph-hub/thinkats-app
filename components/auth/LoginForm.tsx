...
export default function LoginForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Optional: remember workspace hint locally for future logins
      if (workspace.trim()) {
        try {
  ...
  return (
    <div className="rounded-2xl bg-slate-900/70 border border-slate-800 px-6 py-8 shadow-xl backdrop-blur">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label
            htmlFor="workspace"
            className="block text-xs font-medium uppercase tracking-wide text-slate-300 mb-1.5"
          >
            Organisation / Workspace (optional)
          </label>
          <input
            ...
            className="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
            value={workspace}
            onChange={(e) => setWorkspace(e.target.value)}
          />
          <p className="mt-1 text-[11px] text-slate-400">
            Helps us route you to the right tenant/workspace in future.
          </p>
        </div>
        ...
        <p className="mt-3 text-[11px] text-slate-400 text-center">
          ...
          <a href="/request-workspace" className="text-sky-400 hover:text-sky-300">
            Request a workspace
          </a>
        </p>
      </form>
